...
export default function ColorContextExperiment() {
  ...
  const shownBackgrounds = isolated ? ["#888888", "#888888"] : colors;
  const reset = () => { setPair(0); setIsolated(false); setSwapped(false); setJoined(false); setRevealed(false); };
  return <main className={s.page} lang="ko">
    ...
    <header className={s.heading}><h1>A Color Is<br /><em>Never Alone.</em></h1><p>먼저 중앙의 두 색을 비교하세요.<br />주변을 바꾼 뒤 값을 확인할 수 있습니다.</p></header>
    <div className={s.controls} aria-label="색 비교 조작">
      ...
      <button type="button" aria-pressed={revealed} onClick={() => setRevealed(v => !v)}>색값 {revealed ? "숨기기" : "공개하기"}</button>
    ...
    <figure className={s.figure}>
      ...
      <figcaption>
        {revealed
          ? <><span>표본 A/B · <code>{sample}</code></span><span>배경 L/R · <code>{shownBackgrounds[0]}</code> / <code>{shownBackgrounds[1]}</code></span></>
          : <><span>색값은 아직 공개하지 않음</span><span>같은 크기 · 주변만 조작</span></>}
      </figcaption>
    </figure>
    ...
    <footer className={s.footer}><p>먼저 눈으로 비교하고, 그다음 색값을 공개하거나 배경을 통일해 보세요. 연결 띠는 같은 값을 드러내는 보조 도구입니다. 띠 자체도 주변을 바꾸므로 독립된 검증은 아닙니다.</p><p>세 배경쌍은 측색하거나 명도·채도를 등가화한 자극이 아니라 저자가 고른 화면 예시입니다. 화면·조명·색 지각에 따라 인상은 달라집니다. 차이를 느끼지 않아도 실패가 아닙니다. 이 작품은 시력 검사나 사용자 연구 결과가 아닙니다.</p></footer>
  </main>;
}
