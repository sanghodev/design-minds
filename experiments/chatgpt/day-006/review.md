...
- Supervised browser preview: the GitHub-only checkout's first start attempt failed because `next dev` rejected the service's Vite-style flags. After the preserved Sites adapter was merged into an isolated publication branch, the bounded second attempt started successfully. The selected browser environment then became unavailable during navigation, before DOM inspection or capture. No unsanctioned browser substitute was used.
