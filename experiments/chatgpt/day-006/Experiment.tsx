...
export default function ScaleExperiment() {
  ...
  const plot = (baseline: number, name: string) => {
    ...
    return <svg viewBox="0 0 620 310" fill="currentColor" role="img" aria-describedby="scale-summary" aria-label={`${name}. Vertical range ${baseline} to 55. Values 48, 49, 48, 51, 50, 52. Net change 4 units.`}>
  ...
  return <main className={s.page}>
    ...
    <section className={s.instrument} aria-label="Axis range experiment">
      <div className={`${s.views} ${compare ? s.comparing : ""}`}>
        <div className={s.chart}><div className={s.caption}><span>ADJUSTABLE VIEW</span><span>Units · {floor}–55</span></div>{floor > 0 && <p className={s.omission}>Vertical display omits 0–{floor - 1}.</p>}{plot(floor,"Adjustable line chart")}</div>
      ...
      <div className={s.controls}><label htmlFor="axis-floor">Lower axis bound <output>{floor} units</output></label><input id="axis-floor" type="range" min="0" max="47" step="1" value={floor} onChange={e=>setFloor(Number(e.target.value))}/><div className={s.buttons}><button onClick={()=>{setFloor(47);setCompare(true);}}>Compare 47–55 with 0–55</button><button onClick={()=>{setFloor(0);setCompare(false);}}>Reset</button></div><label className={s.check}><input type="checkbox" checked={compare} onChange={e=>setCompare(e.target.checked)}/> Keep 0–55 reference visible</label></div>
    </section>
    <p id="scale-summary" className={s.statement} role="status">The values have not changed. A to F: <strong>+4 units</strong>. The adjustable view shows {ceiling-floor} vertical units{floor > 0 ? ` and omits ${floor} units below its lower bound` : " from zero"}. A visible range does not neutralize its perceptual effect.</p>
