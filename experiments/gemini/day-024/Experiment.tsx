...
export default function PunchCuttingExperiment() {
  ...
  // Cross-section & Matrix Inspector Toggles
  const [showPrandtlSlip, setShowPrandtlSlip] = useState<boolean>(false);
  const [showSmeijersMatrix, setShowSmeijersMatrix] = useState<boolean>(false);
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
          ...
          {/* Academic Inspector Modals */}
          <button
            onClick={() => setShowPrandtlSlip(!showPrandtlSlip)}
            style={{
              padding: '6px 10px',
              backgroundColor: showPrandtlSlip ? '#3b82f6' : '#282b33',
              border: `1px solid ${showPrandtlSlip ? '#60a5fa' : '#3f4450'}`,
              color: showPrandtlSlip ? '#ffffff' : '#93c5fd',
              borderRadius: '4px',
              cursor: 'pointer',
              fontSize: '11px',
              fontWeight: 600,
          ...
          >
            {lang === 'ko' ? '프란틀 슬립라인' : 'Prandtl Slip-Lines'}
          </button>

          <button
            onClick={() => setShowSmeijersMatrix(!showSmeijersMatrix)}
            style={{
              padding: '6px 10px',
              backgroundColor: showSmeijersMatrix ? '#8b5cf6' : '#282b33',
              border: `1px solid ${showSmeijersMatrix ? '#a78bfa' : '#3f4450'}`,
              color: showSmeijersMatrix ? '#ffffff' : '#c4b5fd',
              borderRadius: '4px',
              cursor: 'pointer',
              fontSize: '11px',
              fontWeight: 600
          ...
          >
            {lang === 'ko' ? '스마이어스 b·d·p·q' : 'Smeijers b/d/p/q'}
          </button>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* Right Column: Stage Specific Interactive Controls & Academic Inspectors */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {/* PRANDTL SLIP-LINE CROSS SECTION MODAL / CARD */}
            {showPrandtlSlip && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontWeight: 700, color: '#60a5fa', fontSize: '13px' }}>
                    {lang === 'ko' ? '프란틀 슬립라인 소성 전단 단면 (Prandtl Indentation Field)' : 'Prandtl Plastic Slip-Line Cross-Section'}
                  </span>
                  <button
                    onClick={() => setShowPrandtlSlip(false)}
                    style={{ background: 'none', border: 'none', color: '#93c5fd', cursor: 'pointer', fontSize: '14px' }}
                  >✕</button>
                </div>
                <div style={{ fontSize: '11px', color: '#bfdbfe', lineHeight: 1.6, marginBottom: '10px' }}>
                  {lang === 'ko'
                    ? '압입 하중 P = (2+π)·k ≈ 1,335 MPa 초과 시, 금속 결정은 45° 대수 나선형 슬립라인을 따라 측면으로 방사형 유동하며 테두리에 소성 융기 립(Plastic Lip)을 형성합니다.'
                    : 'When indentation pressure exceeds P = (2+π)·k ≈ 1,335 MPa, steel grains flow along 45° logarithmic spiral shear fans, cold-displacing into raised perimeter lips.'}
                </div>
                <div style={{
                  height: '70px',
                  ...
                  τ_max = σ_y / √3 ≈ 260 MPa · Fan Angle = 45° · Bulge Volume ΔV = 1.18 mm³
                </div>
              </div>
            )}

            {/* SMEIJERS FAMILY MATRIX INSPECTOR (b, d, p, q) */}
            {showSmeijersMatrix && (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <span style={{ fontWeight: 700, color: '#c4b5fd', fontSize: '13px' }}>
                    {lang === 'ko' ? '프레드 스마이어스의 카운터펀치 공유 체계 (b · d · p · q)' : 'Fred Smeijers Counter-Punch Sharing Matrix'}
                  </span>
                  <button
                    onClick={() => setShowSmeijersMatrix(false)}
                    style={{ background: 'none', border: 'none', color: '#c4b5fd', cursor: 'pointer', fontSize: '14px' }}
                  >✕</button>
                </div>
                <div style={{ fontSize: '11px', color: '#ddd6fe', lineHeight: 1.6, marginBottom: '10px' }}>
                  {lang === 'ko'
                    ? '단 하나의 카운터펀치로 찍어낸 내부 안공(골드)을 공통 적용함으로써, 네 글자의 내부 체적과 광학적 리듬이 수학적으로 100% 동일하게 유지됩니다.'
                    : 'A single counter-punch carves the identical internal negative void (gold) across b, d, p, and q, guaranteeing unified typographic rhythm.'}
                </div>
                <div style={{
                  display: 'grid',
                  ...
                  {['b', 'd', 'p', 'q'].map((glyph) => (
                    <div key={glyph} style={{
                      ...
                      <div style={{ fontFamily: '"Cinzel", serif', fontSize: '36px', fontWeight: 700, color: '#fef08a' }}>
                        {glyph}
                      </div>
                      <div style={{ fontSize: '9px', color: '#a78bfa', fontFamily: 'monospace' }}>
                        SHARED VOID
                      </div>
                    </div>
                  ))}
