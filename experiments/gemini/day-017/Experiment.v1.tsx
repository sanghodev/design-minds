...
export default function NonEquilibriumDissipativeTypography() {
  ...
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      // 5. Canvas Rendering: Thermal Heatmap & Convective Streamlines
      ...
      if (canvas) {
        ...
        if (ctx) {
          ...
          // Render Margin Heat Radiators (Top & Bottom Sinks)
          const marginAlpha = Math.min(0.8, (meanT - 20) / 60);
          ctx.fillStyle = `rgba(6, 182, 212, ${marginAlpha * 0.35})`;
          ctx.fillRect(0, 0, canvas.width, 24); // Top cold plate
          ctx.fillRect(0, canvas.height - 24, canvas.width, 24); // Bottom cold plate

          // Draw Bénard Convection Streamlines if super-critical
          if (isFarFromEquilibrium) {
            cellCenters.forEach((cx, idx) => {
              ...
              ctx.strokeStyle = dir > 0 ? "rgba(249, 115, 22, 0.35)" : "rgba(6, 182, 212, 0.35)";
              ctx.lineWidth = 1.8;
              ctx.setLineDash([8, 6]);
              ctx.stroke();

              // Convective vortex center marker
              ctx.fillStyle = dir > 0 ? "rgba(239, 68, 68, 0.6)" : "rgba(56, 189, 248, 0.6)";
              ctx.beginPath();
              ctx.arc(cx, 240, 4, 0, Math.PI * 2);
              ctx.fill();
              ctx.restore();
            });
          }
          ...
          heatSourcesRef.current.forEach((src) => {
            ...
            ctx.strokeStyle = `rgba(251, 146, 60, ${src.intensity * 0.7})`;
            ctx.lineWidth = 2.0;
            ctx.stroke();
          });
        }
