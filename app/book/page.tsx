...
const records=experiments.filter(e=>e.status!=="scaffold-seed"&&e.notebook).sort((a,b)=>a.day-b.day||a.mind.localeCompare(b.mind));
export default function BookPage() {
  ...
  <div className={s.actions}><a href="/research/chatgpt-book.md" download>ChatGPT 통합 원고 다운로드 ↓</a><a href="/research/gemini-book.md" download>Gemini 통합 원고 다운로드 ↓</a><PrintButton/></div>
  <p className={s.note}>현재 {records.length}편의 초고가 연결되어 있습니다. ChatGPT Day 001–003은 2026-09-05에 기존 실험을 다시 읽어 쓴 소급 원고입니다. 도판 수집·인간 편집·사실 검토를 마친 최종 출판본은 아닙니다.</p>
  ...
  <p className={s.kicker}>{e.mind.toUpperCase()} · DAY {String(e.day).padStart(3,"0")} · {e.date} · {b.chapter}</p><h2>{e.title}</h2><p className={s.note}>{b.status} · 기록 {e.notebook!.recorded}</p>{e.status==="research-only"&&<p className={s.note}>Gemini가 작성한 원문 초고입니다. 실행 코드가 아직 전달되지 않아 구현·측정에 관한 서술은 별도 확인이 필요합니다.</p>}<blockquote>{b.hook}</blockquote><p>{b.scene}</p><h3>이 실험이 묻는 것</h3><p>{b.argument}</p><h3>다르게 읽으면</h3><p>{b.counterpoint}</p><h3>독자가 해볼 실험</h3><p>{b.readerExercise}</p><h3>시대의 신호와 가능한 미래</h3><p>{b.futureSignal}</p><h3>다시 확인할 것</h3><p>{b.revisit}</p><h3>도판·권리 기록</h3><p>{b.figurePlan}</p><p>{b.rights}</p><div className={s.actions}><Link href={"/research/"+e.mind+"/day-"+String(e.day).padStart(3,"0")}>연구 방법과 출처 →</Link><Link href={"/"+e.mind+"/day-"+String(e.day).padStart(3,"0")}>{e.status==="published" ? "실험 열기 ↗" : "작품 준비 상태 →"}</Link></div></section>;})}
  </article></main>;
}
