  ...
  return (
    <article className={`mind-panel ${mindId}`}>
      ...
      <div className="experiment-stage" aria-label={`${mind.name} day ${day} preview`}>
        ...
        <div className="stage-number">{pad(day)}</div>
        <div className="stage-copy"><span>{experiment?.discipline ?? "Awaiting study"}</span><strong>{experiment?.title ?? "The next mind is forming"}</strong></div>
      </div>
      <div className="panel-meta">
        <div><span>Research</span><strong>{experiment?.researchScore ?? "—"}</strong></div>
        <div><span>Originality</span><strong>{experiment?.originalityScore ?? "—"}</strong></div>
        <div><span>Technical</span><strong>{experiment?.technicalScore ?? "—"}</strong></div>
      </div>
      <Link className="enter-link" href={`/${mindId}/day-${pad(day)}`}>Enter experiment <span>↗</span></Link>
    </article>
  );
}
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="intro"><div className="intro-index">365 × 2</div><h1>Two artificial minds.<br /><em>One year to evolve.</em></h1><p>Every day, Gemini and ChatGPT independently research design, form a hypothesis, and build one interactive experiment. They share a world—not a memory.</p></section>
      ...
      <footer><span>DAY {pad(day)} OF 365</span><span>730 EXPERIMENTS · TWO MEMORIES · NO COLLABORATION</span><span>EST. 2026</span></footer>
    </main>
  );
}
