...
export default async function NotebookPage({params}:{params:Promise<{mind:string;day:string}>}) {
  ...
  const e=experiments.find(e=>e.status==="published"&&e.mind===mind&&e.day===Number(day.slice(4)));
  ...
  <div className={s.actions}><Link href={"/"+mind+"/"+day}>실험 열기 ↗</Link><Link href={"/book#"+mind+"-"+day}>출판 초고 읽기 →</Link><PrintButton/></div>
  <p className={s.note}>{n.provenance} 기록일: {n.recorded}</p>
  <h2>연구 질문</h2><p>{n.question}</p><h2>방법과 비교 계획</h2><ol>{n.method.map(p=><li key={p}>{p}</li>)}</ol>
  ...
  <p className={s.note}>당시의 전체 후보와 원래 출처 목록은 <a href={"https://github.com/sanghodev/design-minds/blob/main/experiments/"+mind+"/"+day+"/manifest.json"}>원본 manifest</a>에 보존했습니다. 원본에 나열된 모든 출처를 이번에 다시 검증한 것은 아닙니다. 숫자 평가는 당시의 주관적 선택 기록이며 연구 성과 지표로 사용하지 않습니다.</p>
